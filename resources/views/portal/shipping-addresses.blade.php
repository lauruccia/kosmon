...
<section class="card light-card">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div>
            ...
            <h2 style="font-size:20px;font-weight:700;color:var(--ink);margin:4px 0 6px;">I tuoi indirizzi</h2>
...
<section class="card light-card">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;">
        <div style="min-width:0;">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px;">
                @if($indirizzo->label)
                    <strong style="color:var(--ink);font-size:15px;">{{ $indirizzo->label }}</strong>
                @endif
                @if($indirizzo->is_default)
                    <span class="pill" style="background:var(--success-soft);color:var(--success);">Predefinito</span>
                @endif
            </div>
            <p style="font-size:13.5px;color:var(--ink-soft);line-height:1.6;margin:0;">
        ...
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
            ...
            <form method="POST" action="{{ route('portal.shipping-addresses.default', $indirizzo) }}">
                ...
                <button type="submit" style="background:none;border:none;color:var(--info);font-size:12.5px;font-weight:600;cursor:pointer;padding:4px 2px;">
                    Rendi predefinito
                </button>
            </form>
            @endunless

            <button type="button" style="background:none;border:none;color:var(--info);font-size:12.5px;font-weight:600;cursor:pointer;padding:4px 2px;"
            ...
            <form method="POST" action="{{ route('portal.shipping-addresses.destroy', $indirizzo) }}"
                ...
                @method('DELETE')
                @if($ritorno)<input type="hidden" name="redirect_to" value="{{ $ritorno }}">@endif
                <button type="submit" style="background:none;border:none;color:var(--danger);font-size:12.5px;font-weight:600;cursor:pointer;padding:4px 2px;">
                    Elimina
                </button>
            </form>
        </div>
    ...
    <div id="modifica-{{ $indirizzo->id }}" style="display:none;margin-top:18px;padding-top:18px;border-top:1px solid var(--line);">
...
<section class="card light-card" style="text-align:center;padding:36px 24px;">
    ...
    <h3 style="font-size:17px;font-weight:700;color:var(--ink);margin:0 0 6px;">Nessun indirizzo salvato</h3>
...
<section class="card light-card">
    ...
    <h3 style="font-size:17px;font-weight:700;color:var(--ink);margin:4px 0 16px;">Aggiungi un indirizzo</h3>
    ...
    <form method="POST" action="{{ route('portal.shipping-addresses.store') }}">
        ...
        <label style="display:flex;gap:9px;align-items:center;margin-top:14px;font-size:13.5px;color:var(--ink-soft);cursor:pointer;">
