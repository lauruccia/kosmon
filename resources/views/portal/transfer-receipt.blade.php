...
<div class="receipt-wrap">
    ...
    <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:10px;margin-top:18px;">
        ...
        {{-- Web Share API (mostra solo se supportata, via JS) --}}
        <button id="webShareBtn"
                style="display:none;align-items:center;gap:8px;padding:10px 20px;border-radius:50px;
                       background:var(--primary,#0f52c4);color:#fff;font-size:14px;font-weight:600;
        ...
        {{-- WhatsApp fallback --}}
        <a id="waShareBtn" href="https://wa.me/?text={{ $waText }}" target="_blank" rel="noopener"
    ...
    <script>
    (function () {
        const shareText = {{ Js::from($shareText) }};
        const shareUrl  = window.location.href;

        if (navigator.share) {
            // Web Share API disponibile: mostra il pulsante nativo, nascondi WA
            document.getElementById('webShareBtn').style.display = 'inline-flex';
            document.getElementById('waShareBtn').style.display  = 'none';
        }
