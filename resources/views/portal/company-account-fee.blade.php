...
<div>
    ...
    <div class="card" style="padding:28px;">
        ...
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
            ...
            <div>
                <div style="font-size:18px;font-weight:800;color:var(--ink);">Quota di apertura conto</div>
                <div style="font-size:13px;color:var(--ink-soft);margin-top:2px;">Una tantum, per il conto aziendale.</div>
            </div>
        </div>
        ...
        <div class="notice" style="margin-bottom:22px;">
            &Egrave; il costo di apertura del conto aziendale, e si paga <strong>una volta sola</strong>.
            @if(($kyCredit ?? 0) > 0)
                Pagandola in euro ricevi <strong>{{ ky_format($kyCredit) }} KY</strong> sul conto.
            @else
                Pagandola in euro il saldo KY del conto non viene toccato: non &egrave; una ricarica.
            @endif
            @isset($metodi['ky'])
                In alternativa puoi saldarla con il <strong>saldo KY</strong>, e allora il conto va sotto di
                {{ ky_format($amountCents) }} KY
                @if($kyAllowance ?? true)
                    con il massimale aumentato dello stesso importo &mdash; il fido che hai resta intero.
                @else
                    usando il fido che hai gi&agrave;: se non basta, l'addebito non passa e devi pagare in euro.
                @endif
            @endisset
        </div>
        ...
        {{-- Bonifico gia' chiesto: niente bottoni come la prima volta. L'utente
             ha in mano una causale e sta aspettando; qui deve poter solo
             rivedere i dati o cambiare idea. --}}
        @isset($bonifico)
        @if($bonifico)
            <div style="border:1px solid #fcd34d;background:#fffbeb;border-radius:12px;padding:18px;margin-bottom:18px;">
                ...
                <div style="display:grid;gap:10px;">
                    <form method="POST" action="{{ route('portal.company-account-fee.bank-transfer') }}">
                        @csrf
                        <button type="submit" class="cta" style="width:100%;">Procedi con il bonifico (rivedi i dati)</button>
                    </form>

                    <form method="POST" action="{{ route('portal.company-account-fee.bank-transfer.abandon') }}"
        ...
        <div style="display:grid;gap:12px;">
            ...
            @isset($metodi['stripe'])
            <form method="POST" action="{{ route('portal.company-account-fee.stripe') }}">
                @csrf
            ...
            @isset($metodi['paypal'])
            <div id="paypal-button-container" data-create-url="{{ route('portal.company-account-fee.paypal-create-order') }}"></div>
            @endisset
            ...
            @isset($metodi['ky'])
            @php
                $avvisoKy = 'Il conto andrà sotto di ' . ky_format($amountCents) . ' KY'
                    . (($kyAllowance ?? true) ? '' : ', usando il fido che hai già')
                    . '. Confermi?';
            @endphp
            <form method="POST" action="{{ route('portal.company-account-fee.ky') }}"
                  onsubmit="return confirm('{{ $avvisoKy }}');">
                @csrf
        ...
        <div style="margin-top:24px;padding-top:18px;border-top:1px solid var(--border);font-size:13px;color:var(--ink-muted);">
            Nel frattempo il conto continua a funzionare normalmente: puoi pagare, incassare e vendere
            come sempre.
        </div>
...
(function () {
    ...
    paypal.Buttons({
        ...
        onApprove: function () {
            ...
            window.location = '{{ url('/quota-apertura-conto/paypal/capture') }}/' + box.dataset.paymentUuid;
        }
