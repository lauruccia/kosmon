...
<body class="{{ !(auth()->user()?->canAccessBackoffice()) ? 'has-bottom-nav' : '' }}">
    ...
    <div class="app-shell">
        ...
        <main class="content-shell">
            ...
            {{-- Quota di iscrizione da saldare (31/08/2026): l'utente entra e
                 vede tutto, ma finche' non paga non invia, non incassa e non
    ...
    {{-- Overlay: Sessione scaduta (419/401 su mobile) --}}
    <div id="km-session-expired" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.48);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);align-items:center;justify-content:center;padding:24px;">
        <div style="background:var(--bg,#fff);border-radius:24px;padding:36px 28px;width:min(100%,360px);box-shadow:0 24px 64px rgba(0,0,0,.25);text-align:center;">
            <div style="width:64px;height:64px;background:var(--bg-soft,#f1f5f9);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:30px;">&#x23F1;</div>
            <h2 style="margin:0 0 10px;font-size:22px;font-weight:700;color:var(--ink,#1e293b);">Sessione scaduta</h2>
            <p style="margin:0 0 24px;font-size:14px;color:var(--ink-soft,#64748b);line-height:1.6;">La tua sessione e scaduta per inattivita.<br>Ricarica la pagina per continuare da dove eri rimasto.</p>
            <button onclick="window.location.reload()" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:14px;background:var(--primary,#0b2244);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;margin-bottom:10px;">Ricarica la pagina</button>
            <button onclick="window.location.href='/'" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:14px;background:transparent;color:var(--ink-soft,#64748b);border:1.5px solid var(--border,#e2e8f0);border-radius:12px;font-size:15px;font-weight:500;cursor:pointer;">Torna alla home</button>
            <p style="margin:16px 0 0;font-size:12px;color:var(--ink-muted,#94a3b8);">La pagina si ricarichera automaticamente tra <strong id="km-session-countdown">10</strong> secondi.</p>
        </div>
    </div>
    <script>
    (function () {
        ...
        function attachAxios(ax) {
            ax.interceptors.response.use(function (r) { return r; }, function (err) {
        ...
        else { window.addEventListener('DOMContentLoaded', function () { if (window.axios) attachAxios(window.axios); }); }
        ...
        document.addEventListener('visibilitychange', function () {
            if (document.hidden) { hiddenAt = Date.now(); }
                ...
                hiddenAt = null;
                _nativeFetch('/dashboard', { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
                    .then(function (res) { if (res.status === 401 || res.status === 419) showExpired(); })
                    .catch(function () {});
            } else { hiddenAt = null; }
        });
