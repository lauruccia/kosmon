...
### Lotto 3 (admin) — chiuso il buco di autorizzazione alla radice
Ricognizione per l'estrazione FormRequest degli admin ha rivelato un buco di privilege-escalation piu' ampio del lotto 1: `AdminFeeController`, `AdminKyCardController`, `AdminBroadcastController` (inviare broadcast a tutti!) **senza alcuna guardia**, e `CompanyController::suspendCompany/unsuspendCompany/cancelPaymentPlan/cancelNettingProposal` senza `authorizeBackoffice` (un utente qualsiasi poteva sospendere qualunque azienda). Causa radice: **nessuna rotta admin aveva un middleware di backoffice a livello di gruppo**.
**Fix sistemico:** aggiunto `->middleware('backoffice')` a tutte le **119** rotte `/admin` in `routes/web.php` (gating esplicito per-rotta, robusto anche per rotte admin future). I controlli piu' fini nei controller (es. `is_super_admin` per emissione KY, `authorizePermission` per ruoli) restano come gate interni aggiuntivi. Test `BackofficeAccessGuardTest` esteso (+9 rotte nel provider + test sospensione azienda da non-admin). La redundanza con l'HasMiddleware dei 4 controller del lotto 1 e' innocua (doppio check). NB: le rotte morte `/admin/support` ora sono anche backoffice-gated (restano da implementare/rimuovere).
